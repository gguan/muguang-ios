...
#define ARCVIEW_DEFAULT_FONT_SIZE   20.0
#define ARCVIEW_DEFAULT_RADIUS      150.0
#define ARCVIEW_DEFAULT_ARC_SIZE    180
...
- (id)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        self.font = [UIFont systemFontOfSize:ARCVIEW_DEFAULT_FONT_SIZE];
        self.text = @"CoreTextDemo";
...
- (void)setArcSize:(CGFloat)arcSize {
    _arcDegree = arcSize * M_PI/180.0;
}

//get arc size in degrees
-(CGFloat)arcSize{
    return _arcDegree * 180.0/M_PI;
}
...
- (void)drawRect:(CGRect)rect {
    ...
    PrepareGlyphArcInfo(line, glyphCount, glyphArcInfo, _arcDegree);
    ...
    if(ARCVIEW_DEBUG_MODE){
        ...
        CGContextAddArc(context, 0.0, 0.0, self.radius, M_PI_2 + _arcDegree / 2.0, M_PI_2 - _arcDegree / 2.0, 1);
        CGContextSetRGBStrokeColor(context, 1.0, 0.0, 0.0, 1.0);
        CGContextStrokePath(context);
    }
    
    // Rotate the context 90 degrees counterclockwise (per 180 degrees)
    CGContextRotateCTM(context, _arcDegree/2.0);
    ...
    // 画图片
//    UIImage *brushImage = [UIImage imageNamed:@"aaa.png"];
//    CGFloat img_width = brushImage.size.width;
//    [brushImage drawAtPoint:CGPointMake(textPosition.x - img_width * 0.7, textPosition.y)];
